...
namespace camera
{
  ...
  int scaled_tile_dim_width = TILE_DIM;
  int scaled_tile_dim_height = TILE_DIM;
  int moved_x = 0;
  int moved_y;
  ...
  void scale(int zoom)
  {
    ...
    float height_ratio = z_window_height/(float)WINDOW_HEIGHT;
    float width_ratio = z_window_width/(float)WINDOW_HEIGHT;

    camera::scaled_tile_dim_width = TILE_DIM* (float) width_ratio;
    camera::scaled_tile_dim_height = TILE_DIM* (float) height_ratio;
  };
