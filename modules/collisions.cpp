...
namespace collisions
{
  ...
  void _resolve_solid_collisions()
  {
    ...
    for(int i=0; i<collisions::distances.size(); i++)
    {
      ...
      if(collisions::distances[i].is_solid){
        ...
        for (auto const& [k, v]: entity::entities.at(entity_id).sensors)
        {
          // entity on the left
  ...
  void _set_sensors(int entity_id)
  {
    ...
    for(int i=0; i < collisions::SENSOR_COUNT; i++)
    {
      ...
      switch(i) {
        case SENSOR_TOP:
            s.x = entity::entities.at(entity_id).mid_x;
            s.y = (entity::entities.at(entity_id).pos.y - collisions::SENSOR_OFFSET);
            s.id = SENSOR_TOP;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_TOP, s));
        break;
        case SENSOR_TOP_RIGHT:
            s.x = (entity::entities.at(entity_id).pos.x + entity::entities.at(entity_id).dims.w + collisions::SENSOR_OFFSET);
            s.y = (entity::entities.at(entity_id).pos.y  - collisions::SENSOR_OFFSET);
            s.id = SENSOR_TOP_RIGHT;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_TOP_RIGHT, s));
        break;
        case SENSOR_RIGHT:
            s.x = (entity::entities.at(entity_id).pos.x + entity::entities.at(entity_id).dims.w  + collisions::SENSOR_OFFSET);
            s.y = (entity::entities.at(entity_id).pos.y) ;
            s.id = SENSOR_RIGHT;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_RIGHT, s));
        break;
        case SENSOR_BOTTOM_RIGHT:
            s.x = (entity::entities.at(entity_id).pos.x + entity::entities.at(entity_id).dims.w + collisions::SENSOR_OFFSET);
            s.y = (entity::entities.at(entity_id).pos.y + entity::entities.at(entity_id).dims.h + collisions::SENSOR_OFFSET);
            s.id = SENSOR_BOTTOM_RIGHT;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_BOTTOM_RIGHT, s));
        break;
        case SENSOR_BOTTOM:
            s.x = entity::entities.at(entity_id).mid_x;
            s.y = (entity::entities.at(entity_id).pos.y + entity::entities.at(entity_id).dims.h + collisions::SENSOR_OFFSET);
            s.id = SENSOR_BOTTOM;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_BOTTOM, s));
        break;
        case SENSOR_BOTTOM_LEFT:
            s.x = entity::entities.at(entity_id).pos.x - collisions::SENSOR_OFFSET;
            s.y = (entity::entities.at(entity_id).pos.y + entity::entities.at(entity_id).dims.h + collisions::SENSOR_OFFSET);
            s.id = SENSOR_BOTTOM_LEFT;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_BOTTOM_LEFT, s));
        break;
        case SENSOR_LEFT:
            s.x = entity::entities.at(entity_id).pos.x - collisions::SENSOR_OFFSET ;
            s.y = entity::entities.at(entity_id).mid_y;
            s.id = SENSOR_LEFT;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_LEFT, s));
        break;
        case SENSOR_TOP_LEFT:
            s.x = entity::entities.at(entity_id).pos.x - collisions::SENSOR_OFFSET;
            s.y = entity::entities.at(entity_id).pos.y - collisions::SENSOR_OFFSET;
            s.id = SENSOR_TOP_LEFT;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_TOP_LEFT, s));
        break;
        case SENSOR_CENTER:
            s.x = entity::entities.at(entity_id).mid_x;
            s.y = entity::entities.at(entity_id).mid_y;
            s.id = SENSOR_CENTER;
            entity::entities.at(entity_id).sensors.insert(std::pair<int, collisions::Sensor>(SENSOR_CENTER, s));
        break;
      }  
    }
  }
  ...
  {
    entity::entities.at(entity_id).abs.clear();
    for(int a = 0; a < collisions::ABS_COUNT; a++)
    {
      collisions::AABB aabb;
      aabb.min_x = (entity::entities.at(entity_id).pos.x + collisions::SENSOR_OFFSET);
      aabb.min_y = (entity::entities.at(entity_id).pos.y + collisions::SENSOR_OFFSET);
      aabb.max_y = (entity::entities.at(entity_id).pos.y + entity::entities.at(entity_id).dims.h - collisions::SENSOR_OFFSET);
      aabb.max_x = (entity::entities.at(entity_id).pos.x + entity::entities.at(entity_id).dims.w - collisions::SENSOR_OFFSET);
      aabb.id = AABB_FULL;
      entity::entities.at(entity_id).abs.insert(std::pair<int, collisions::AABB>(AABB_FULL, aabb));
